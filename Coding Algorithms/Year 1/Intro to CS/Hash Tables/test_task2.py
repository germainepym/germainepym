...
# my test case
  def test_load_dictionary_me(self):
    with self.vis():
      table = task1.HashTable()
      task2.load_dictionary(table, "words_simple.txt")
      self.assertEqual(table.count,6)

    with self.vis():
      table = task1.HashTable()
      task2.load_dictionary(table, "words_dup.txt")
      self.assertEqual(table.count,5)

    with self.vis():
      table = task1.HashTable()
      with self.assertRaises(Exception, msg = 'Exception should be raised when time_limit is exceeded'):
        task2.load_dictionary(table,"words_simple.txt", 0)
